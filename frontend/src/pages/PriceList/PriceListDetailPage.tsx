...
export default function PriceListDetailPage() {
  ...
  const rows = data
    ? (onlyChanged ? data.diff.filter((r) => r.delta !== null && r.delta !== 0) : data.diff)
    : [];
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-[1600px] mx-auto space-y-8">
      ...
      {!loading && stats && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <StatCard icon={<TrendingUp size={18} className="text-rose-500" />} label="Naik" value={stats.naik} color="rose" />
          <StatCard icon={<TrendingDown size={18} className="text-emerald-600" />} label="Turun" value={stats.turun} color="emerald" />
          <StatCard icon={<Minus size={18} className="text-secondary" />} label="Tetap" value={stats.tetap} color="secondary" />
          <StatCard icon={<Sparkles size={18} className="text-tertiary" />} label="Baru" value={stats.baru} color="tertiary" />
        </div>
      )}
...
function StatCard({ icon, label, value, color }: { icon: React.ReactNode; label: string; value: number; color: string }) {
  const colorMap: Record<string, string> = {
    ...
    tertiary:  "bg-tertiary/5 border-tertiary/20",
  };
  return (
    <div className={`card p-4 border ${colorMap[color] ?? ""} flex items-center gap-3`}>

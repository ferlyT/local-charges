    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm">
      ...
      <div className="absolute top-0 left-0 right-0 p-4 flex items-center justify-between text-white z-10 bg-gradient-to-b from-black/50 to-transparent">
        ...
        <div className="flex items-center gap-4">
          {isImage && (
            <button onClick={() => setRotation(r => r + 90)} className="hover:text-secondary mr-2" title="Rotate">
              <RotateCw size={20} />
            </button>
          )}
          {(isImage || isPdf) && (
            <>
              <button onClick={() => setZoom(z => Math.max(0.5, z - 0.25))} className="hover:text-secondary">
                <ZoomOut size={20} />
              </button>
              <span className="text-xs">{Math.round(zoom * 100)}%</span>
              <button onClick={() => setZoom(z => Math.min(3, z + 0.25))} className="hover:text-secondary">
                <ZoomIn size={20} />
              </button>
            </>
          )}
          <button onClick={handleDownload} className="hover:text-secondary ml-2" title="Download">
            <Download size={20} />
          </button>
          <button onClick={onClose} className="hover:text-secondary ml-4" title="Close">
            <X size={24} />
          </button>
        </div>
      </div>
      ...
      <div className="w-full h-full flex items-center justify-center p-12 overflow-auto">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center text-secondary">
            ...
            <button onClick={handleDownload} className="mt-4 px-[20px] py-[12px] bg-tertiary text-on-primary rounded-md hover:opacity-90 transition-opacity">
              Download File
            </button>
          </div>
        )}

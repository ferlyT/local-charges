...
import { 
  ...
  Clock, ArrowRight, Activity, Trash2, Shield, Upload,
  DollarSign, History, FileSpreadsheet, BarChart3
} from 'lucide-react';
import { useTranslation } from '../../../hooks/useTranslation';

const inspectionStatusMap: Record<string, { label: string; className: string; key: any }> = {
...
export default function Dashboard() {
  ...
  // Price list uploads summary
  const [plUploads, setPlUploads] = useState<UploadRow[]>([]);
  const [plTotal, setPlTotal]     = useState<number>(0);
  const [plLoading, setPlLoading] = useState(false);

  const canReadPricelist = hasPermission(currentUser, 'pricelist:read');

  useEffect(() => {
    if (!canReadPricelist) return;
    setPlLoading(true);
    api.get('/pricelist/uploads?page=1&pageSize=4')
      .then((res) => {
        setPlUploads(res.data.rows ?? []);
        setPlTotal(res.data.total ?? 0);
      })
      .catch(console.error)
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-[1600px] mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        {/* Price List Stat Card */}
        {canReadPricelist && (
          <div className="card p-6 border-l-4 border-l-amber-500 flex flex-col justify-between sm:col-span-2 xl:col-span-1">
            <div className="flex items-start justify-between mb-4">
              <div>
                <div className="text-sm font-semibold text-secondary uppercase tracking-wider mb-1">Price List Upload</div>
                <div className="text-4xl font-bold text-primary">
                  {plLoading ? <div className="h-10 w-24 skeleton mt-1" /> : plTotal}
                </div>
              </div>
            ...
            <div className="flex items-center justify-between border-t border-secondary/10 pt-4">
              <span className="text-sm text-secondary">Total upload price list</span>
              <Link
                to="/pricelist"
                className="flex items-center gap-1 text-xs font-semibold text-amber-600 hover:text-amber-500 transition-colors"
              >
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          ...
          {canReadPricelist && (
            <Link to="/pricelist" className="group card p-5 flex flex-col items-center justify-center text-center hover:bg-amber-500 hover:text-white transition-all duration-300">
              <div className="w-12 h-12 rounded-full bg-amber-500/10 text-amber-500 flex items-center justify-center mb-3 group-hover:bg-white/20 group-hover:text-white transition-all">
                <BarChart3 size={24} />
              </div>
              <span className="font-semibold text-sm group-hover:text-white transition-colors">Tren Harga</span>
            </Link>
          )}
          {hasPermission(currentUser, 'pricelist:upload') && (
            <Link to="/pricelist/upload" className="group card p-5 flex flex-col items-center justify-center text-center hover:bg-orange-500 hover:text-white transition-all duration-300">
              <div className="w-12 h-12 rounded-full bg-orange-500/10 text-orange-500 flex items-center justify-center mb-3 group-hover:bg-white/20 group-hover:text-white transition-all">
                <Upload size={24} />
              </div>
              <span className="font-semibold text-sm group-hover:text-white transition-colors">Upload Price List</span>
            </Link>
          )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pt-4">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Link to="/local-charges" className="text-sm font-semibold text-tertiary hover:text-tertiary/80 flex items-center gap-1 group">
                    ...
                    recentLC.data.map(charge => {
                      ...
                      return (
                        <tr key={charge.fdId} className="hover:bg-neutral/30 transition-colors group">
                          ...
                          <td className="px-4 py-3 text-sm text-secondary truncate max-w-[120px]">
                            {charge.details?.[0]?.fdNamaCustomer || '-'}
                          </td>
                          <td className="px-4 py-3 text-sm whitespace-nowrap">
                            <span className={stat.className}>{stat.key ? (t as any)(stat.key) : stat.label}</span>
                          </td>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Link to="/inspection-reports" className="text-sm font-semibold text-emerald-500 hover:text-emerald-500/80 flex items-center gap-1 group">
              {t('dashboard_view_all')} <ArrowRight size={14} className="group-hover:translate-x-1 transition-transform" />
            </Link>
          </div>
          <div className="card overflow-hidden">
        ...
        {canReadPricelist && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold text-primary flex items-center gap-2">
                <FileSpreadsheet size={20} className="text-amber-500" /> Price List Terbaru
              </h2>
              <Link to="/pricelist" className="text-sm font-semibold text-amber-500 hover:text-amber-500/80 flex items-center gap-1 group">
                {t('dashboard_view_all')} <ArrowRight size={14} className="group-hover:translate-x-1 transition-transform" />
              </Link>
            </div>
            <div className="card overflow-hidden">
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-secondary/15">
                  <thead className="bg-neutral/40">
                    <tr>
                      <th className="px-4 py-3 text-left text-[0.7rem] font-semibold text-secondary uppercase tracking-wider">Berlaku Mulai</th>
                      <th className="px-4 py-3 text-left text-[0.7rem] font-semibold text-secondary uppercase tracking-wider">Baris</th>
                      <th className="px-4 py-3 text-left text-[0.7rem] font-semibold text-secondary uppercase tracking-wider">Status</th>
                      <th className="px-4 py-3 text-right"></th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-secondary/10">
                    {plLoading ? (
                      ...
                      plUploads.map(row => (
                        <tr key={row.id} className={`hover:bg-neutral/30 transition-colors group ${row.isSuperseded ? 'opacity-60' : ''}`}>
                          <td className="px-4 py-3 whitespace-nowrap">
                            <div className="text-sm font-bold text-primary">
                              {new Date(row.effectiveDate).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}
                            </div>
                            {row.isSuperseded && (
                              <div className="text-[0.65rem] text-secondary/50 font-semibold uppercase tracking-wide">Digantikan</div>
                            )}
                          </td>
                          ...
                          <td className="px-4 py-3 text-right">
                            <Link to={`/pricelist/uploads/${row.id}`} className="text-secondary hover:text-amber-500 p-1 inline-block opacity-0 group-hover:opacity-100 transition-opacity">
                              <Eye size={18} />
                            </Link>
                          </td>
                        </tr>
                      ...
                      <tr>
                        <td colSpan={4} className="p-8 text-center">
                          <div className="flex flex-col items-center gap-2">
                            <History size={28} className="text-secondary/30" />
                            <p className="text-secondary text-sm">Belum ada upload price list</p>
                            {hasPermission(currentUser, 'pricelist:upload') && (
                              <Link to="/pricelist/upload" className="text-xs font-semibold text-amber-600 hover:underline mt-1">

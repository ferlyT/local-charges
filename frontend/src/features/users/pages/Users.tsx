...
import { Users as UsersIcon, CheckSquare, AlertTriangle, Shield, User, Search, Clock } from 'lucide-react';
    ...
    <div className="p-4 sm:p-6 md:p-8 max-w-[1400px] mx-auto space-y-8">
      ...
      <div className="bg-surface shadow-md border border-secondary/20 rounded-lg overflow-hidden transition-all duration-300">
        ...
        <div className="md:hidden divide-y divide-secondary/15">
          {filteredUsers?.map((user) => (
            <div key={user.fdId} className="px-4 py-4 space-y-3">
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  <span className="font-semibold text-primary truncate">{user.fdNama}</span>
                  {user.fdId === currentUser?.id && (
                    <span className="shrink-0 bg-tertiary/10 text-tertiary px-2 py-0.5 rounded-full text-[0.65rem] font-bold tracking-wide font-mono uppercase">
                      {t('users_badge_you')}
                    </span>
                  )}
                </div>
                {user.fdAktif ? (
                  <span className="shrink-0 inline-flex items-center gap-1 text-[0.65rem] font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded-full uppercase tracking-wider">
                    <CheckSquare size={11} /> {t('users_status_active')}
                  </span>
                ) : (
                  <span className="shrink-0 inline-flex items-center gap-1 text-[0.65rem] font-semibold text-rose-600 dark:text-rose-400 bg-rose-500/10 border border-rose-500/20 px-2 py-0.5 rounded-full uppercase tracking-wider">
                    <AlertTriangle size={11} /> {t('users_status_inactive')}
                  </span>
                )}
              </div>

              <div className="flex items-center justify-between gap-3 text-[0.82rem]">
                <span className="text-secondary font-mono truncate">{user.fdUsername}</span>
                <span className="flex items-center gap-1 text-secondary/70 shrink-0">
                  <Clock size={12} />
                  {new Date(user.fdCreatedAt).toLocaleDateString(language === 'id' ? 'id-ID' : 'en-US', {
                    year: 'numeric',
                    month: 'short',
              ...
              <div className="flex items-center gap-2 pt-1">
                <select
                  ...
                  onChange={(e) => handleChangeRole(user, parseInt(e.target.value))}
                  disabled={user.fdId === currentUser?.id || changeRoleMutation.isPending}
                  className="flex-1 min-w-0 bg-surface border border-secondary/20 text-secondary text-sm rounded-lg focus:ring-tertiary/50 focus:border-tertiary/50 p-2 outline-none cursor-pointer disabled:opacity-50"
                >
                  {roles?.map(role => (
                    <option key={role.fdId} value={role.fdId}>{role.fdNama}</option>
                  ))}
                ...
                <button
                  onClick={() => handleToggleStatus(user)}
                  disabled={user.fdId === currentUser?.id || toggleStatusMutation.isPending}
                  className={`shrink-0 text-[0.7rem] font-bold uppercase tracking-wider px-3 py-2 rounded-md transition-all duration-150 border active:scale-[0.97] ${
                    user.fdAktif 
                      ? 'text-rose-600 border-rose-500/20 bg-rose-500/5 hover:bg-rose-500/10 dark:text-rose-400' 
                      : 'text-emerald-600 border-emerald-500/20 bg-emerald-500/5 hover:bg-emerald-500/10 dark:text-emerald-400'
                  } disabled:opacity-30 disabled:cursor-not-allowed`}

 ...
 *     (dianggap langsung baris kategori).
 ...
 *
 * Kalau struktur file berubah signifikan (blok tidak diawali "BY ", atau
 * baris tujuan tidak ditemukan), parser tetap menyimpan rawSnapshot penuh
 * per sheet supaya data upload tidak hilang, dan status upload akan
 * ditandai PARTIAL/FAILED agar bisa dicek manual dari rawSnapshot.
 */
...
function colAHasCategoryShape(text: string): boolean {
  // heuristik: baris kategori biasanya kalimat/nama barang (huruf, spasi, kurung),
  // sedangkan baris transit biasanya pendek & mengandung simbol "±" atau angka+hari.
